...
export default function ImportantTasks() {
  ...
  const STORAGE_KEY = "important_task_manager_data";
  ...
  const markAsNormal = (taskId) => {
    const taskToMove = tasks.find((task) => task.id === taskId);
    if (!taskToMove) return;

    const updatedImportantTasks = tasks.filter((task) => task.id !== taskId);
    saveTasks(updatedImportantTasks);

    const todoStorageKey = "task_manager_data";
    const existingTodos =
      JSON.parse(localStorage.getItem(todoStorageKey)) || [];

    const updatedTodos = [
      ...existingTodos,
      { ...taskToMove, status: "normal" },
    ];
    localStorage.setItem(todoStorageKey, JSON.stringify(updatedTodos));
    toast.success("Task moved to normal tasks!");
  };
    ...
    <div className="flex w-full">
      ...
      <div className="flex flex-col gap-4 w-full max-w-5xl mx-auto py-6 px-4">
        ...
        <div className="bg-white shadow-md rounded-lg p-4">
          <h2 className="text-xl font-bold mb-4">Important Tasks</h2>
          {tasks.length === 0 ? (
              ...
              .map((task) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-2 items-end gap-1 ml-4 group-hover:flex hidden">
                      <button
